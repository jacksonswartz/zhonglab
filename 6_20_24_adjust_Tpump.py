...
# read timing:: just extract all wait_time values based on file naming
wait_times = np.zeros(len(csv_files))
for i, path in enumerate(csv_files):
    path = os.path.normpath(path).split(os.sep) #convert string path into ordered list of path components
    wait_str = path[-2] #take only the second-to-last component of path (everything before 0p64ms/TEK0000)
    wait_str = wait_str.replace('p', '.') #replace 'p' (point) with '.'
...
ax.set_title(rf"6/20 - Pump Time Change")
