...
csv_files.append(DATA_DIR + "/TEK0014.CSV")
...
csv_files_freq.append(DATA_DIR + "/TEK0015.CSV")
...
print('ligui')
plt.plot(dfs[0]['Seconds'], dfs[0]['Volts'])
...
#08/01 update: set start and stop indices by identifying leftmost, rightmost peaks
#Only use TEK0000 file: indices should be the same for the other files

start_idx = find_peaks(-1*dfs[0]["Volts"][150:350], prominence=PROMINENCE, distance=DISTANCE)[0]
stop_idx = find_peaks(-1*dfs[0]["Volts"][2200:2400], prominence=PROMINENCE, distance=DISTANCE)[0]

if len(start_idx) != 1 or len(stop_idx) != 1:
        print('Warning: found wrong number of peaks. Got ' + str(len(start_idx)) + ' start-peaks and ' + str(len(stop_idx)) + ' stop-peaks.')

print('mario')
print(start_idx)
print(stop_idx)
center_idx = (start_idx + stop_idx) // 2
...
for i in range(len(all_scan_freq)): #separate combs into different plots
    ...
    if PLOT_OD:
        ...
        ax.set_ylabel("Transmission (nW)")
    ax.set_title("AFC " + date + " - Slide" + str(i+1))
