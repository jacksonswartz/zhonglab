...
DATA_DIR = ("/Users/JacksonS/Documents/zhonglab"
            "/07_23_24/10mK_12Amp/pumpnprobe/tr2_196042p746GHz/changing_T_wait/Tpump_25p6ms__apump_0p9")
...
for i, df in enumerate(dfs): #7/17: adjusted code since peaks do not occur RIGHT at scan edge
    ...
    peaks = find_peaks(df["Volts"], #using scipy "find_peaks" function, and only taking the x-value of the obtained peaks
                       prominence=0.07, distance=DISTANCE)[0] #prominence: increase from adjacent point, #distance: minimum distance between neighboring peaks
...
params.add('tau_slow', value=10, min=0)
params.add('offset', value=0)
result_fit_area = model_area.fit(list(map(lambda x: x.params['amplitude'].value, all_hole_results)), x=all_hole_centers, params=params)
print("")
...
if PLOT_OD:
    ...
    ax.set_title(rf"Wait Time Change - 7/23")
    ...
    if LOG_CMAP:
        ...
        cb.set_label(r"Time $T_{wait}$ (ms)")
...
if PLOT_OD_SPLIT:
    ...
    for i in range(2):
        ...
        axs[i].set_title('$T_{wait} = $ ' + f'{t_wait[indices[i]]:.4g}' + ' to ' + f'{t_wait[indices[i+1]-1]:.4g}' + ' ms')
    ...
    #else:
        #ax.set_ylabel("Transmission (nW)")
    fig.suptitle(rf"Wait Time Change - 7/23")
...
if PLOT_SINGLE_SCAN_HOLES:
    for i in range(len(dfs)):
        ...
        plt.title('7/23 Hole: $T_{wait} = $' + str(t_wait[i]) + ' ms')
...
if PLOT_LINEWIDTHS:
    ...
    if LOG_SCALE:
        title = "Hole Linewidth (FWHM, Log Scale) vs Wait Time - 7/23"
    else:
        title = "Hole Linewidth (FWHM) vs Wait Time"
    ax.set_title(title)
    ax.set_xlabel("Wait Time (s)")
...
if PLOT_AREA:
    ...
    if LOG_SCALE:
        title = "Hole Area (Log Scale) versus Time - " + str(date)
    else:
        title = "Hole Area versus Time - " + str(date)
    ax.set_title(title)
    ax.set_xlabel("Time (s)")
...
if PLOT_CONTRAST_OD:
    ...
    if LOG_SCALE:
        title = "Hole Contrast OD (Log Scale) vs Wait Time - " + str(date)
    else:
        title = "Hole Contrast OD vs Wait Time- " + str(date)
    ax.set_title(title)
    ax.set_xlabel("Wait Time (s)")
